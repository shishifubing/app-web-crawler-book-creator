...
public class Chapter extends InnerFile implements Runnable {
    ...
    public Chapter(Book epubBook, ZipOutputStream zos, String url, int index, String title, Content content, Toc toc) {
    ...
    public void run() {
        ...
        while (chapter == null) {
            ...
            try {
                Response response = Jsoup.connect(url).cookies(epubBook.cookies()).execute();
        ...
        addContent("    \r\r\n\n\n<p>Chapter source: <a href=\"" + url + "\">" + url + "</a></p>\r\n");
        addContent("</body>\r\n");
        addContent("</html>");
        synchronized (zos) {
            addToZip(zos);
    ...
    private boolean lineIsValid(String line, String chapterTitle) {
        if (line.replaceAll("[^a-zA-Z]", "").startsWith(chapterTitle) || line.startsWith("[/expand]")
                || line.startsWith("[caption id=") || line.replaceAll("[^a-zA-Z]", "").contains(chapterTitle)) {
            return false;
        }
        switch (line) {
            case "Next Chapter":
